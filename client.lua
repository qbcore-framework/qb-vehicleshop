...
local function getVehPrice()
    return comma_value(
        QBCore.Shared.Vehicles[Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle]["price"]
    )
end
...
local function setClosestShowroomVehicle()
    ...
    for id in pairs(Config.Shops[closestShop]["ShowroomVehicles"]) do
        local dist2 =
            #(pos -
            vector3(
                Config.Shops[closestShop]["ShowroomVehicles"][id].coords.x,
        ...
        function()
            ...
            while inTestDrive do
                if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                    ...
                    if
                        secondsLeft >= tonumber(1000 * testDriveTime) - 20 or
                            GetPedInVehicleSeat(NetToVeh(testDriveVeh), -1) ~= PlayerPedId()
                     then
                    ...
                    drawTxt(
                        Lang:t("general.testdrive_timer") .. math.ceil(testDriveTime - secondsLeft / 1000),
                        4,
                        0.5,
                        0.93,
                        0.50,
    ...
    if not Config.UsingTarget then
        for i = 1, #Config.Shops[shopName]["ShowroomVehicles"] do
            zones[#zones + 1] =
                BoxZone:Create(
                vector3(
                    Config.Shops[shopName]["ShowroomVehicles"][i]["coords"].x,
        ...
        combo:onPlayerInOut(
            function(isPointInside)
                if isPointInside then
                    if
                        PlayerData and PlayerData.job and
                            (PlayerData.job.name == Config.Shops[insideShop]["Job"] or
                                Config.Shops[insideShop]["Job"] == "none")
                     then
                            ...
                            vehicleMenu = {
                                {
                                    ...
                                    header = getVehBrand():upper() ..
                                        " " .. getVehName():upper() .. " - $" .. getVehPrice()
                                },
                                {
            ...
            for k in pairs(Config.Shops) do
                for i = 1, #Config.Shops[k]["ShowroomVehicles"] do
                    ...
                    local veh =
                        CreateVehicle(
                        model,
                        Config.Shops[k]["ShowroomVehicles"][i].coords.x,
                        Config.Shops[k]["ShowroomVehicles"][i].coords.y,
                        Config.Shops[k]["ShowroomVehicles"][i].coords.z,
                        false,
                        false
                    )
...
-- Events
RegisterNetEvent(
    "qb-vehicleshop:client:homeMenu",
    function()
...
RegisterNetEvent(
    "qb-vehicleshop:client:showVehOptions",
    function()
        exports["qb-menu"]:openMenu(vehicleMenu)
    end
)
...
RegisterNetEvent(
    "qb-vehicleshop:client:TestDrive",
    function()
            ...
            QBCore.Functions.TriggerCallback(
                ...
                function(netId)
                    ...
                    QBCore.Functions.Notify(
                        Lang:t(
                            "general.testdrive_timenoti",
                            {testdrivetime = Config.Shops[tempShop]["TestDriveTimeLimit"]}
                        )
                    )
                end,
                Config.Shops[tempShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle,
                Config.Shops[tempShop]["TestDriveSpawn"],
                true
            )
...
RegisterNetEvent(
    "qb-vehicleshop:client:customTestDrive",
    function(data)
...
RegisterNetEvent(
    "qb-vehicleshop:client:TestDriveReturn",
    function()
...
RegisterNetEvent(
    "qb-vehicleshop:client:openVehCats",
    function(data)
        local vehMenu = {
            {
                header = Lang:t("menus.goback_header"),
...
RegisterNetEvent(
    "qb-vehicleshop:client:openFinance",
    function(data)
        local dialog =
        ...
        if dialog then
            if not dialog.downPayment or not dialog.paymentAmount then
                return
            end
            TriggerServerEvent(
...
RegisterNetEvent(
    "qb-vehicleshop:client:openCustomFinance",
    function(data)
        ...
        if dialog then
            if not dialog.downPayment or not dialog.paymentAmount or not dialog.playerid then
                return
            end
...
RegisterNetEvent(
    "qb-vehicleshop:client:swapVehicle",
    function(data)
...
RegisterNetEvent(
    "qb-vehicleshop:client:buyShowroomVehicle",
    function(vehicle, plate)
        ...
        QBCore.Functions.TriggerCallback(
            ...
            vehicle,
            Config.Shops[tempShop]["VehicleSpawn"],
            true
        )
    end
)
...
RegisterNetEvent(
    "qb-vehicleshop:client:getVehicles",
    function()
        QBCore.Functions.TriggerCallback(
            "qb-vehicleshop:server:getVehicles",
            function(vehicles)
                local ownedVehicles = {}
...
RegisterNetEvent(
    "qb-vehicleshop:client:getVehicleFinance",
    function(data)
        local vehFinance = {
            {
                header = Lang:t("menus.goback_header"),
...
RegisterNetEvent(
    "qb-vehicleshop:client:financePayment",
    function(data)
        local dialog =
            exports["qb-input"]:ShowInput(
            {
        ...
        if dialog then
            if not dialog.paymentAmount then
                return
            end
...
RegisterNetEvent(
    "qb-vehicleshop:client:openIdMenu",
    function(data)
        local dialog =
            exports["qb-input"]:ShowInput(
            {
